<?php
	...
	$departmentQuery = $conn->prepare('SELECT id FROM department WHERE name = ?');
	$departmentQuery->bind_param("s", $departmentName);
	...
	$query->bind_param("sssi", $_REQUEST['firstName'], $_REQUEST['lastName'], $_REQUEST['email'], $departmentID);
	$query->execute();
